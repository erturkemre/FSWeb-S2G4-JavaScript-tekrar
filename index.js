...
function uceBolunenler() {
  ucetambolunenler = [];
  sayilar.forEach(bolunen);
  function bolunen(item) {
    if (item % 3 === 0) {
...
//3c çözümü:
function uceBolunenlerToplami(uceBolunenler) {
  ucebolunenlerintoplami = uceBolunenler.reduce((total, item) => {
    return (total += item);
  }, 0);
  return ucebolunenlerintoplami;
}
console.log(uceBolunenlerToplami(uceBolunenler()));
